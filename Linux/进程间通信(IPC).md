...
- 重入/不可重入函数：竞态条件
    - 如果一个函数在多个执行流的竞态条件下运行，如果多个执行流的争抢不会对其他执行流造成影响，这个函数就被称为可重入函数，否则是一个不可重入函数
    - 如果一个函数内部，涉及到对全局数据的修改，并且这个操作时不受保护的非原子的，那么这个函数就是一个不可重入函数，否则就是可重入函数。
    - 意味着，我们以后写函数时，一定要考虑函数的可重入与不可重入的问题

- volatile关键字
    - 用于修饰一个变量，保持这个变量的内存可见性，防止编译器过度优化
- SIGCHLD
    - 我们之前避免僵尸进程是子进程之后就进行进程等待，一直等待时因为不知道什么时候操作系统的通知到来，如果知道子进程的退出通知到来那么就不需要等待了，操作系统时通过一个SIGCHLD信号来通知父进程，在信号的自定义函数中循环非阻塞等待，